...
int main()
{
	...
	vec r[N],s,v[N],E,B;
	double rs[N],va,vb,m[N],ma,mb,dt,a,q[N],Emag,Etheta,Ephi,Bmag,Btheta,Bphi,k,vc;
    bool c;
    ofstream file;
    ...
    Etheta=0;
    Ephi=PI/2;
    E=Emag*vec(cos(Etheta)*sin(Ephi),sin(Etheta)*sin(Ephi),cos(Ephi));
    Bmag=1;
    Btheta=-PI/2;
    Bphi=PI/2;
    B=Bmag*vec(cos(Btheta)*sin(Bphi),sin(Btheta)*sin(Bphi),cos(Bphi));
    ...
    file<<"C = "<<vc<<endl;
    file<<"Emag = "<<Emag<<endl;
    file<<"Etheta = "<<Etheta<<endl;
    file<<"Ephi = "<<Ephi<<endl;
    file<<"E = "<<E<<endl;
    file<<"Bmag = "<<Bmag<<endl;
    file<<"Btheta = "<<Btheta<<endl;
    file<<"Bphi = "<<Bphi<<endl;
    file<<"B = "<<B<<endl;
    file<<"v0 = { "<<va<<" , "<<vb<<" }"<<endl;
    file<<"m = { "<<ma<<" , "<<mb<<" }"<<endl;
...
void int_dt(vec* r, vec* v, vec& E, vec& B, double* q, double* m, double k, double c, double dt, int n)
{
    for(int i=0;i<n;i++)
    {
        v[i]+=((qf(i,r,q,k,n)+q[i]*(E+(v[i]%(B+bs(i,r,v,q,k,c,n)))))/m[i])*dt;
        r[i]+=v[i]*dt;
    }
}
